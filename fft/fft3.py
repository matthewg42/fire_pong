...
def main():
    global out, means, inp, puff_frequency, threshold
    setup()
    while True:
        l,data = inp.read()
        inp.pause(1)
        if l:
            try:
                ...
                if target_density > puff_density and threshold > min_threshold:
                    ...
                    threshold += threshold_step
                print '%s | %5.3f | %5.3f' % (s, puff_density, threshold)
...
def calculate_levels(data, chunk,sample_rate):
    ...
    # Find amplitude
    power = np.log10(np.abs(fourier))**2
    # Araange array into len(puffers) frequency bands
    power = np.reshape(power,(len(puffers),chunk/len(puffers)))
...
def setup():
    global inp, out, means, puff_frequency
    for _ in range(0, len(puffers)):
